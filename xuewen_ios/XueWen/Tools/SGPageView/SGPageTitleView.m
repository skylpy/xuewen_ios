...
-(void)setUpDelegate:(id<SGPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames
{
...
- (UIView *)bottomSeparator {
    if (!_bottomSeparator) {
        ...
        _bottomSeparator.backgroundColor = DefaultBgColor;
    }
...
//            [btn setBackgroundColor:ColorA(204, 208, 225, 0.2)];
...
- (void)changeIndicatorViewLocationWithButton:(UIButton *)button {
    if (self.allBtnWidth <= self.bounds.size.width) { /// SGPageTitleView 不可滚动
        [UIView animateWithDuration:_indicatorAnimationTime animations:^{
            ...
            }else if (self.indicatorLengthStyle == SGIndicatorLengthTypeCustom) {
                self.indicatorView.SG_width = _indicatorWidth;
            }
...
- (void)setIndicatorLengthStyle:(SGIndicatorLengthType)indicatorLengthStyle {
    ...
    if (indicatorLengthStyle == SGIndicatorLengthTypeEqual) {
        CGFloat firstBtnTextWidth = [self SG_widthWithString:firstBtn.currentTitle font:[UIFont fontWithName:kRegFont size:SGPageTitleViewTextFont]];
        ...
        CGFloat firstBtnIndicatorWidth = [self SG_widthWithString:firstBtn.currentTitle font:[UIFont fontWithName:kRegFont size:SGPageTitleViewTextFont]] + SGIndicatorTypeSpecialMultipleLength;
        ...
        if (_indicatorWidth) {
            indicatorViewW = _indicatorWidth;
            indicatorViewX = 0.2 * (firstBtn.SG_width - firstBtnTextWidth);
        }else {
