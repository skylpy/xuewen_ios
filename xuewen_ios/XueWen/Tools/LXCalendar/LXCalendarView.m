...
-(void)responData{
    ...
    for (int i = 0; i <42; i++) {
        ...
        if (i >= firstWeekday && i < (firstWeekday + totalDays)) {
            ...
            NSString * startdd = [NSDate dateFormTimestamp:model.startDay withFormat:@"dd"];
            NSString * enddd = [NSDate dateFormTimestamp:model.endDay withFormat:@"dd"];
            
            NSString * startMM = [NSDate dateFormTimestamp:model.startDay withFormat:@"MM"];
            NSString * endMM = [NSDate dateFormTimestamp:model.endDay withFormat:@"MM"];
            
            //开始日期
            if ([startMM integerValue] == model.month&&[startdd integerValue] == model.day) {
                model.isStartday = YES;
            }
            
            //结束日期
            if ([endMM integerValue] == model.month&&[enddd integerValue] == model.day) {
                model.isEndday = YES;
            }
            
            model.cycleBackColor = [UIColor whiteColor];
...
//    model.cycleBackColor = [UIColor colorWithRed:231/255.0 green:243/255.0 blue:255/255.0 alpha:1];
    model.startDay = self.startDay;
    model.endDay = self.endDay;
...
-(LXCalendarWeekView *)calendarWeekView{
    if (!_calendarWeekView) {
        _calendarWeekView =[[LXCalendarWeekView alloc]initWithFrame:CGRectMake(0, self.calendarHeader.lx_bottom, self.lx_width, 20)];
...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        flow.itemSize = CGSizeMake(self.lx_width/7, 32);
        _collectionView =[[UICollectionView alloc]initWithFrame:CGRectMake(0, self.calendarWeekView.lx_bottom, self.lx_width, 5 * 32) collectionViewLayout:flow];
