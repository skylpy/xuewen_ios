...
-(void)setModel:(LXCalendarDayModel *)model{
    ...
    NSString * endD = [NSDate dateFormTimestamp:model.endDay withFormat:@"yyyy-M-dd"];
    NSString * start = [NSDate dateFormTimestamp:model.startDay withFormat:@"yyyy-M-dd"];
    NSString * endds = [NSString stringWithFormat:@"%ld-%ld-%ld",model.year,model.month,model.day];
...
//
//        }
        self.label.textColor = model.currentMonthTitleColor?model.currentMonthTitleColor:Color(@"#000000");
        if (model.isToday) {
            [self.label setFTCornerdious:self.label.lx_width/2];
            self.label.textColor = model.todayTitleColor?model.todayTitleColor:[UIColor redColor];
            self.label.backgroundColor = Color(@"#3699FF");
        }
    }
    self.interLabel.text = @"";
    if (model.isEndday) {
        
        self.interLabel.text = @"终";
        self.label.textColor = [UIColor orangeColor];
        self.interLabel.textColor = [UIColor orangeColor];
    }
    if (model.isStartday) {
        
        self.interLabel.text = @"始";
        self.label.textColor = Color(@"#3699FF");
        self.interLabel.textColor = Color(@"#3699FF");
    }
    self.backgroundColor = model.cycleBackColor;
}
